...
_1back = ['057714622355104447600', //1 double
          ...
          '223466374334611573000'] //1 double
...
var instructions_delay_task = {
  ...
  pages: ['instructions/Screenshot (227).png',
  'instructions/Screenshot (228).png',
  'instructions/Screenshot (229).png',
  'instructions/Screenshot (230).png',
  'instructions/Screenshot (231).png',
  'instructions/Screenshot (232).png'], 
  audio: ['recordings/SLP Instruction Recordings/delay_task/ST_11.mp3',
...
function check_correctness(ISI_response){
  ...
  if (response == 1){
    ...
    else if (levels[main_task][n-1][block][trial_index]  == levels[main_task][n-1][block][trial_index-n]){
      return true
    }
    else{
      return false
  ...
  else if (response == 0) { console.log(trial_index-n)
        if (trial_index - n < 0){
          return true
        }
        else if (levels[main_task][n-1][block][trial_index]  == levels[main_task][n-1][block][trial_index-n]){
...
var feedback_trial = {
  on_start: function(trial){
    ...
    if (result == true && response == 1){ //it was same
    ...
    else if (result == true && response == 0){ //it was different
    ...
    else if (result == false && response == 1){ //it was different 
      trial.stimulus = `<div><img src="shapes for delay task/red_ex.png" style = "width:17%"></img>
        <img src="shapes for delay task/green_rectangle.png" style = "width:25%; height:15% ; position:absolute; top: 45%; right: 0.5%"></div>`
    }
    else if (result == false && response == 0){ // it was same
      trial.stimulus = `<div><img src="shapes for delay task/red_ex.png" style = "width:17%"></img>
        <img src="shapes for delay task/green_rectangle.png" style = "position:absolute; width:25%; height:15% ;top: 45%; left: 4%"> </div>`
    }
...
var display_n_back__practice_trial = {
  ...
  loop_function: function(data){
    ...
    trial_index = 0
    var last_block_mistakes = jsPsych.data.get().last(3*levels[main_task][n-1][block].length).filter({"correct": false}).count()
...
var n_back_timeline ={
  timeline: [instructions_delay_task,
            ...
            display_n_back_trial, inter_n_message, 
            display_n_back_trial, inter_n_message,
            display_n_back_trial, inter_n_message, post_delay_task]
}
