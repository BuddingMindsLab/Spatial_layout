...
var instruction_images = ['instructions/Screenshot (195).png',
...
function create_timeline_variables(){
  ...
  if(group == '2'){
    ...
    for (var i = 0; i < set1.length; i++){
      ...
      overlapping: true, layout: 'Y', test: false, phase: "phase2"})
      test_variables.push({correct_response: set1[i], image_set: set1, map: 'stimuli/blue world.png', 
      overlapping: false, layout: 'B', test: true, phase: "test"})
  }}
  if(group == '3'){
  ...
  if(group == '5'){
    ...
    for (var i = 0; i < set1.length; i++){
      ...
      overlapping: false, layout: 'Y', test: false, phase: "phase1"})
      phase2_variables.push({correct_response: set2[i], image_set: set2, map: 'stimuli/blue world.png', 
      overlapping: true, layout: 'B', test: false, phase: "phase2"})
      test_variables.push({correct_response: set1[i], image_set: set1, map: 'stimuli/yellow world.png', 
      overlapping: false, layout: 'Y', test: true, phase: "test"})
  }}
  if(group == '6'){
    var set1 = shuffle(E)
    var set2 = shuffle(F)
...
on_start: function(trial){
  ...
  trial.data = {repetition: count_repetitions, phase: jsPsych.timelineVariable("phase", true)}}
  else{
    trial.data = {repetition: count_repetitions, exclusion: true, phase: jsPsych.timelineVariable("phase", true)}
  }
},
...
timeline_variables: [{correct_response: practice_set[0], image_set: practice_set, map: 'stimuli/pink world.png', 
      overlapping: false, layout: 'P', test: false, phase: "practice main task"},
