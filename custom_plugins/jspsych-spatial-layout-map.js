...
jsPsych.plugins["spatial-layout-map"] = (function () {
  ...
  plugin.trial = function (display_element, trial) {
    ...
    var html =
      ...
      trial.background +
      '" id="background" width="100%" height="100%"' +
      `>
      <img src="` +
    ...
    if (trial.overlapping) {
      for (var i = 1; i < trial.images.length + 1; i++) {
        html +=
          ` <img class="flipCard" src="` +
          trial.disabled_button_image +
          `" id="` +
          layout +
          `-btn` +
          i +
    ...
    function end_trial() {
      ...
      if (!trial.test) {
        var trial_data = {
          ...
          "click-by-click data": {
            "touch locations": response.buttons,
            "touch objects": get_object_array(),
